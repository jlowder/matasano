...
(define (encryption_oracle input)
                                        ...
                                        ;(encrypt-aes-128-cbc random-key random-key (pkcs7-pad-bytes (bytes-append prefix (string->bytes/utf-8 (quote-specials input)) suffix) 16)))
  (encrypt-aes-128-cbc random-key random-key (pkcs7-pad-bytes (string->bytes/utf-8 (quote-specials input)) 16)))

(define (decryption_oracle input)
  ; see if input decrypts successfully by making sure each character is ascii
  (with-handlers ([exn:fail? (lambda (exn)
                               (displayln (exn-message exn))
                               (decrypt-aes-128-cbc random-key random-key input))])
    (if (regexp-match #px"[^[:ascii:]]" (bytes->string/utf-8 (unpad (decrypt-aes-128-cbc random-key random-key input))))
      (display "something doesn't look right\n")
      #f)))
