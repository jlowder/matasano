...
(provide encryption_oracle decryption_oracle random-key random-iv)
...
(define (encryption_oracle input)
                                        ...
                                        ; ctr encrypt with key and return
  (encrypt-aes-128-ctr random-iv random-key (bytes-append prefix (string->bytes/utf-8 (quote-specials input)) suffix)))

(define (decryption_oracle input)
  (if (regexp-match ";admin=true;" (bytes->string/utf-8 (decrypt-aes-128-ctr random-iv random-key input) #\.))
      #t
      #f))
