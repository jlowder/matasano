...
(define (pad-of n [keysize 8])
  (bytes-append (gen-pad (+ (bytes-length n) keysize)) (integer->integer-bytes (* 8 (+ keysize (bytes-length n))) 8 #f #f)))
...
(for/or ([i (in-range 1 100)])
        (printf "checking key length: ~a\n" i)
        (let* ([glue-padding (pad-of original-message i)]
               [forged-sig (md4 new-message
                                (integer-bytes->integer signature #f #f 0 4)
                                (integer-bytes->integer signature #f #f 4 8)
                                (integer-bytes->integer signature #f #f 8 12)
                                (integer-bytes->integer signature #f #f 12 16)
