...
(provide hmac256-bytes hmac256-file)
...
(define (hmac256-file file [key random-key])
  (let* ([fp (open-input-file file #:mode 'binary)]
         [contents (read-bytes 1024000 fp)])
    (close-input-port fp)
    (hmac256-bytes contents key)))

(define (hmac256-bytes message [key random-key])
  (let* ([padded-key (bytes-append key (make-bytes (- 64 (bytes-length key)) 0))]
    ...
    (sha256 (bytes-append opad (sha256 (bytes-append ipad message))))))
